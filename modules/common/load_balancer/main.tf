...
resource "aws_lb" "load_balancer" {
 # name  = substr(format("%s-%s", "${var.prefix_name}-LB", replace(uuid(), "-", "")), 0, 32)
  name  = substr(format("%s-%s", "${var.prefix_name}-LB", random_id.unique-identifier.hex), 0, 32)
...
resource "aws_lb_target_group" "lb_target_group" {
 # name  = substr(format("%s-%s", "${var.prefix_name}-TG", replace(uuid(), "-", "")), 0, 32)
  name  = substr(format("%s-%s", "${var.prefix_name}-TG", random_id.unique-identifier.hex), 0, 32)
  vpc_id              = var.vpc_id
